...
class robot:
    ...
    def __init__(self, length = 20.0, init = 0):
        if(init == 0):
          self.x = random.uniform(-1,2) * world_size # initial x position
          self.y = random.uniform(-1,2) * world_size # initial y position
        else:
    ...
    def sense(self, noise_flag=1):
        ...
        for landmark in landmarks:
            deltaY = landmark[0] - self.y
            deltaX = landmark[1] - self.x
